...
print_success() {
  ...
  echo ""
  echo -e "     ${GREEN}/step1-init${NC}     → Шаг 1: Создание структуры проекта"
  echo -e "     ${GREEN}/step2-brief${NC}    → Шаг 2: Бизнес-анализ требований"
  echo -e "     ${GREEN}/step3-prd${NC}      → Шаг 3: Документ требований (PRD)"
  echo -e "     ${GREEN}/step4-arch${NC}     → Шаг 4: Техническая архитектура"
  echo -e "     ${GREEN}/step5-sprint${NC}   → Шаг 5: Планирование спринта"
  echo -e "     ${GREEN}/step6-validate${NC} → Шаг 6: Валидация и очередь задач"
  echo -e "     ${GREEN}/step7-build${NC}    → Шаг 7: Автономная разработка"
  echo ""
