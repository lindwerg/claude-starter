...
var PreToolUseInputSchema = BaseHookInputSchema.extend({
...
var SessionStartInputSchema = BaseHookInputSchema.extend({
  type: external_exports.enum(["resume", "compact", "clear", "start"])
});
var PreCompactInputSchema = BaseHookInputSchema.extend({
  context_tokens: external_exports.number().optional(),
  max_tokens: external_exports.number().optional()
});
var StopInputSchema = BaseHookInputSchema.extend({
  reason: external_exports.string().optional()
});
var HookOutputSchema = external_exports.object({
  result: external_exports.enum(["continue", "block"]),
...
function checkOutput(outputPattern, projectDir) {
  const fullPath = path2.join(projectDir, outputPattern);
  if (outputPattern.includes("*") || outputPattern.includes("?") || outputPattern.includes("[")) {
    try {
      ...
      const pattern = path2.basename(outputPattern);
      if (fs.existsSync(dir)) {
        const files = fs.readdirSync(dir).filter((f) => minimatch(f, pattern, { dot: true })).map((f) => path2.join(dir, f));
...
function updateTaskStatus(taskId, projectDir) {
  const queuePath = path2.join(projectDir, ".bmad/task-queue.yaml");
...
async function main() {
  ...
  const queuePath = path2.join(projectDir, ".bmad/task-queue.yaml");
  const ralphMarker = path2.join(projectDir, ".bmad/ralph-in-progress");
  ...
  const relativePath = path2.relative(projectDir, filePath);
