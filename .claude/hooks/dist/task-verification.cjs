...
function parseYaml(filePath) {
  try {
    const jsonOutput = (0, import_child_process.execSync)(`yq -o json '.' "${filePath}"`, {
      encoding: "utf8",
...
function runGates(gates, projectDir) {
  for (const gate of gates) {
    if (!gate.required)
      continue;
    try {
      (0, import_child_process.execSync)(gate.command, {
        cwd: projectDir,
        stdio: ["pipe", "pipe", "pipe"],
        timeout: 6e4
...
function updateTaskStatus(taskId, projectDir) {
  ...
  try {
    (0, import_child_process.execSync)(`yq -i '(.tasks[] | select(.id == "${taskId}") | .status) = "done"' "${queuePath}"`, {
      cwd: projectDir,
      stdio: ["pipe", "pipe", "pipe"]
    });
    (0, import_child_process.execSync)(`yq -i '(.tasks[] | select(.id == "${taskId}") | .completed_at) = "${now}"' "${queuePath}"`, {
      cwd: projectDir,
      stdio: ["pipe", "pipe", "pipe"]
    });
    (0, import_child_process.execSync)(`yq -i '.summary.completed_tasks += 1' "${queuePath}"`, {
      cwd: projectDir,
      stdio: ["pipe", "pipe", "pipe"]
    });
    (0, import_child_process.execSync)(`yq -i '.current_task = null' "${queuePath}"`, {
      cwd: projectDir,
      stdio: ["pipe", "pipe", "pipe"]
    });
    return true;
