...
if [ -f "$CLAUDE_PROJECT_DIR/.bmad/ralph-progress.yaml" ]; then
    TASK=$(grep "current_task:" "$CLAUDE_PROJECT_DIR/.bmad/ralph-progress.yaml" 2>/dev/null | head -1 | sed 's/.*current_task: *"\([^"]*\)".*/\1/')
...
if [ -f "$CLAUDE_PROJECT_DIR/.bmad/sprint-status.yaml" ]; then
    SPRINT=$(grep "current_sprint:" "$CLAUDE_PROJECT_DIR/.bmad/sprint-status.yaml" 2>/dev/null | head -1 | sed 's/[^0-9]//g')

    # Get sprint end date
    END_DATE=$(grep -A20 "sprint_number: $SPRINT" "$CLAUDE_PROJECT_DIR/.bmad/sprint-status.yaml" 2>/dev/null | grep "end_date:" | head -1 | sed 's/.*"\([^"]*\)".*/\1/')
...
if [ -f "$CLAUDE_PROJECT_DIR/.bmad/task-queue.yaml" ]; then
    # Count from summary section (more reliable)
    TOTAL=$(grep "total_tasks:" "$CLAUDE_PROJECT_DIR/.bmad/task-queue.yaml" 2>/dev/null | head -1 | sed 's/[^0-9]//g') || TOTAL=0
    DONE=$(grep "completed_tasks:" "$CLAUDE_PROJECT_DIR/.bmad/task-queue.yaml" 2>/dev/null | head -1 | sed 's/[^0-9]//g') || DONE=0
