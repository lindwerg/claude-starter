...
- Functional Requirements (что должно делать)
- Non-Functional Requirements (performance, security, scalability)
- User Stories (кто и как будет использовать)
- Data Requirements (какие данные хранить)
- Integration Requirements (с чем интегрироваться)

**Если файл НЕ найден:**
```
❌ PRD не найден!

Сначала создай требования:
/step3-prd
```
STOP — не продолжай без PRD.

### Шаг 2: Опционально прочитать Product Brief

```bash
# Найти последний product-brief (для контекста)
ls -t docs/product-brief-*.md | head -1
```

Прочитай для понимания бизнес-контекста и целей.
...
**ВАЖНО:** Используй NFRs из PRD чтобы задавать более точные архитектурные вопросы!
...
### Шаг 2: Вызови BMAD команду /architecture
...
**Что происходит:**
1. Команда /architecture проверяет существование `/tmp/step4-answers.yaml`
2. Находит файл → переходит в batch mode
3. Читает YAML напрямую через Read tool (все 37 переменных)
4. Загружает template из `.claude/skills/bmad/bmad-v6/templates/architecture.md`
5. Подставляет переменные в template
6. Генерирует документ `docs/architecture-{project}-{date}.md`
7. **🚀 Выполняет Part 13: автоматически обновляет CLAUDE.md с проектной архитектурой!**
8. Обновляет workflow status
